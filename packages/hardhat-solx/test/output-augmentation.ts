...
// The plugin augments `CompilerOutputBytecode` with the optional solx-only
// `debugInfo` field in `src/type-extensions.ts`. Side-effect-import that file
// directly so the augmentation is visible to typed accesses below.
...
// render Solidity stack traces. This file compiles small contracts with solx
// (downloading the binary on first run) and asserts the field is populated.
//
// solx 0.1.4 weighs ~50MB and is downloaded on first run; the suite is gated
// behind `HARDHAT_DISABLE_SLOW_TESTS=true` to mirror the hardhat repo's other
// compile-heavy tests.

// `\x7fELF` = the four ELF magic bytes that start every ELF binary, hex-
// encoded. solx's debugInfo blob is a hex-encoded ELF32-MSB container — the
// asserted prefix is a cheap sanity check that the field is the wire format
// EDR expects before EDR's gimli/object parser tries to read it.
